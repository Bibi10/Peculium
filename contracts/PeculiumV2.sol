...
contract PeculiumV2 is BurnableToken, Ownable {
    // Our token is a standard ERC20 Token with burnable and ownable aptitude
    ...
    string public name = "PeculiumV2"; //token name
    string public symbol = "PCLV2"; // token symbol
    uint256 public decimals = 8; // token number of decimal
    address internal upgradeAgent;
    ...
    enum UpgradeState { Waiting, Upgrading, Completed}

    event Upgrade(address indexed owner, uint256 amount);
    /*** Others Functions of the contract ***/

    function PeculiumV2(address _upgradeAgent) {
    ...
    function upgradeStatus() public constant returns (UpgradeState) {
        if (totalSupply > 0) {
            return UpgradeState.Upgrading;
        }
        else if (totalSupply == MAX_SUPPLY_NBTOKEN) {
    ...
    function assignTokens(address _to, uint _value) public returns (bool) {
